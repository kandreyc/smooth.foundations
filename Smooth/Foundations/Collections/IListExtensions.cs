...
namespace Smooth.Collections
{
	...
	public static class IListExtensions
    {
	    ...
	    /// <summary>
	    ///     If the specified list is empty, returns an empty option; otherwise, returns an option containing a random element
	    ...
	    ///     However, it has a best case time complexity of O(n) for lists that are already sorted and is quite fast when used
        ...
        {
            for (var right = 1; right < ts.Count; ++right)
            {
                var insert = ts[right];
                var left = right - 1;
