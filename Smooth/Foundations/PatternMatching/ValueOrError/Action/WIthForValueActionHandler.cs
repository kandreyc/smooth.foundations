...
namespace Smooth.Foundations.PatternMatching.ValueOrError.Action
{
    public sealed class WithForValueActionHandler<T>
    {
        ...
        internal WithForValueActionHandler(T value,
        ...
        {
            if (!_isUseless) _values.Add(value);
            return this;
        }
        ...
        {
            if (!_isUseless)
                _addPredicateAndAction(
                    val => _values.Slinq().Any(v => Collections.EqualityComparer<T>.Default.Equals(val, v)), action);
            return _matcher;
        }
    }
}