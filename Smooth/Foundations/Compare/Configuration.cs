...
namespace Smooth.Compare
{
	...
	public class Configuration
    {
	    ...
	    /// <summary>
	    ///     This can be used to override the platform setting and enable or disable automatic comparer creation, which can be
	    ///     quite useful while testing in different environments.
	    /// </summary>
	    ...
	    ///     If you supply a custom configuration and want to apply the default registrations, add a call to
	    ///     base.RegisterComparers() from your method override.
	    /// </summary>
	    ...
	    ///     The default implementation logs warnings on registration collisions, the use of inefficient or invalid comparers,
	    ///     and unregistered find requests for value types if JIT is disabled.
	    /// </summary>
            ...
            switch (eventType)
            {
                case EventType.FindUnregistered:
                    if (NoJit && type.IsValueType)
                        Debug.LogWarning("A " + comparerType.ToStringCached() +
                                         " has been requested for a non-registered value type with JIT disabled, this is a fragile operation and may result in a JIT exception.\nType:" +
                                         type.FullName);
                    break;
                case EventType.InefficientDefault:
                    Debug.LogWarning("A " + comparerType.ToStringCached() +
                                     " has been requested that will perform inefficient comparisons and/or cause boxing allocations.\nType:" +
                                     type.FullName);
                    break;
                case EventType.InvalidDefault:
                    Debug.LogWarning("A " + comparerType.ToStringCached() +
                                     " has been requested for a non-comparable type.  Using the comparer will cause exceptions.\nType:" +
                                     type.FullName);
                    break;
                case EventType.AlreadyRegistered:
                    Debug.LogWarning("Tried to register a " + comparerType.ToStringCached() +
	    ...
	    /// <summary>
	    ///     If JIT is enabled, this method is called by the finder when it is asked to supply a sort order comparer for an
	    ///     unregistered, non-IComparable
	    ///     <T>
	    ///         type.
	    ...
	    /// <summary>
	    ///     If JIT is enabled, this method is called by the finder when it is asked to supply an equality comparer for an
	    ///     unregistered, non-IEquatable
	    ///     <T>
	    ///         type..
