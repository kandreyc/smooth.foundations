...
namespace Smooth.Compare
{
	...
	public static class Finder
    {
                ...
                if (customType != null)
                {
                    if (typeof(Configuration).IsAssignableFrom(customType))
                    {
                        ...
                        if (ctor != null)
                            ...
                            Debug.LogError("A " + customConfigurationClassName +
            ...
            if (type.IsEnum)
                switch (Type.GetTypeCode(type))
                {
                    ...
                    case TypeCode.UInt64:
                        Register(new Blittable64EqualityComparer<T>());
                        break;
                    default:
                        Register(new Blittable32EqualityComparer<T>());
                        break;
                }
            else
	    ...
	    /// <summary>
	    ///     Registers a sort order comparer with the specified comparison and an equality comparer with the specified equals
	    ///     function for type T.
	    /// </summary>
	    ...
	    /// <summary>
	    ///     Registers a sort order comparer with the specified comparison and an equality comparer with the specified equals
	    ///     and hashCode functions for type T.
	    /// </summary>
	    public static void Register<T>(Comparison<T> comparison, DelegateFunc<T, T, bool> equals,
	    ...
	    ///     Note: On platforms without JIT compilation, the supplied comparers should respectively inherit from
	    ///     Smooth.Collections.Comparer
	    ///     <T>
	    ///         and Smooth.Collections.EqualityComparer
	    ...
	    ///     Note: Do not call this method directly as it is part of the internal API and a new comparers may be created on
	    ///     every call.  Use Smooth.Collections.Comparer<T>.Default to get the default sort order comparer.
	    /// </summary>
	    public static IComparer<T> Comparer<T>()
        {
            ...
            {
                OnEvent.Raise(comparerType,
                    type.IsValueType ? EventType.InefficientDefault : EventType.EfficientDefault, type);
                return System.Collections.Generic.Comparer<T>.Default;
            }
	    ...
	    ///     Note: On platforms without JIT compilation, the supplied comparer should inherit from
	    ///     Smooth.Collections.EqualityComparer<T> in order to force the AOT compiler to create the proper generic types.
	    /// </summary>
	    ...
	    ///     Note: Do not call this method directly as it is part of the internal API and a new comparers may be created on
	    ///     every call.  Use Smooth.Collections.EqualityComparer<T>.Default to get the default equality comparer.
	    /// </summary>
	    public static IEqualityComparer<T> EqualityComparer<T>()
        {
            ...
            OnEvent.Raise(comparerType, type.IsValueType ? EventType.InefficientDefault : EventType.EfficientDefault,
                type);
            return System.Collections.Generic.EqualityComparer<T>.Default;
        }
