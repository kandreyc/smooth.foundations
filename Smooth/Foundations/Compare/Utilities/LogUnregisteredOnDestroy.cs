...
#endif
	/// <summary>
	///     Simple utility that listens to finder events and logs all requests for unregistered comparers when the component is
	///     destroyed.
	///     This can be useful when running an application meant for a non-JIT platform in a JIT-enabled simulator so you can
	///     test many code paths, then track down and fix any potential comparer issues.
	/// </summary>
        ...
        protected void OnDestroy()
        {
            ...
            if (comparers.Count > 0 || equalityComparers.Count > 0)
            {
                ...
                if (comparers.Count > 0)
                {
                    ...
                    foreach (var type in comparers) sb.AppendLine(type.FullName);
                }
                ...
                if (equalityComparers.Count > 0)
                {
                    ...
                    foreach (var type in equalityComparers) sb.AppendLine(type.FullName);
                }
