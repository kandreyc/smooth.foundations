...
namespace Smooth.Slinq.Collections
{
	...
	public class Linked<T> : IDisposable
    {
	    ...
	    /// <summary>
	    ///     Traverses the node list that starts with this node, clears the value of each node, and releases the resulting node
	    ///     list to the pool.
	    /// </summary>
	    public void Dispose()
        {
	    ...
	    ///     Note: Nodes should generally not be released one at a time as it will incur much greater overhead than batching
	    ///     releases using LinkedHeadTail<T>s.
	    /// </summary>
	...
	public class Linked<K, T> : IDisposable
    {
	    ...
	    /// <summary>
	    ///     Traverses the node list that starts with this node, clears the key and value of each node, and releases the
	    ///     resulting node list to the pool.
	    /// </summary>
	    public void Dispose()
        {
            var dk = default(K);
	...
	public struct LinkedHeadTail<T> : IEquatable<LinkedHeadTail<T>>
    {
	    ...
	    ///     This list and the specified list must be well formed when calling this method or the program will enter an invalid
	    ///     state, resulting in unspecified behaviour.
	    ...
	    ///     The list must be well formed when calling this method or the program will enter an invalid state, resulting in
	    ...
	    ///     The caller of this method is responsible for making sure the returned Slinq is not used after the nodes are
	    ///     modified or returned to the pool.
	    /// </summary>
	    ...
	    ///     The caller of this method is responsible for making sure the returned Slinq is not used after the nodes are
	    ///     modified or returned to the pool.
	    ///     If backtrack detection is enabled, the supplied backtrack detector can be returned to the pool using its
	    ///     TryReleaseShared method to prevent subsequent enumeration of the returned Slinq.
	    /// </summary>
	    ...
	    ///     Ownership of the nodes contained in the list is transferred to the Slinq.  When the Slinq is disposed, the nodes
	    ///     will be added to the disposal queue.
	    /// </summary>
	    ...
	    /// <summary>
	    ///     Returns a pooled lookup with the default key comparer containing the nodes in the list as partitioned by the
	    ...
	    /// <summary>
	    ///     Returns a pooled lookup with the specified key comparer containing the nodes in the list as partitioned by the
	    ///     specified key selector.
	    ///     Ownership of the nodes contained in this list is transerred to the lookup.
	    /// </summary>
	...
	public static class Linked
    {
	    ...
	    ///     The specified list must be well formed when calling this method or the program will enter an invalid state,
	    ///     resulting in unspecified behaviour.
	    ///     Calling this method will invalidate the specified list and any variables containing its nodes.
	    /// </summary>
	    ...
	    ///     This method uses an introspective merge sort algorithm that will optimally sort rather than split lists with 3 or
	    ///     fewer nodes.
	    ///     The specified list must be well formed when calling this method or the program will enter an invalid state,
	    ///     resulting in unspecified behaviour.
	    ///     Calling this method will invalidate the specified list and any variables containing its nodes.
	    ...
	    public static LinkedHeadTail<T> Sort<T>(LinkedHeadTail<T> input, Comparison<T> comparison, bool ascending)
        {
            ...
            if (input.count == 2)
            {
                if ((ascending
                        ...
                        : comparison(input.tail.value, input.head.value)) <= 0) return input;
            ...
            bool ascending)
        {
            ...
            if (input.count == 2)
            {
                if ((ascending
                        ...
                        : comparison(input.tail.key, input.head.key)) <= 0) return input;
	    ...
	    /// <summary>
	    ///     Merges the specified sorted lists using the specified comparison and ordering.  Elements from the left list will
	    ///     appear before elements from the right on equal comparisons.
	    ///     The specified lists must be well formed when calling this method or the program will enter an invalid state,
	    ///     resulting in unspecified behaviour.
