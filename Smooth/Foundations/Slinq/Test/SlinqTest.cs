	...
	public class SlinqTest : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            Debug.Log("Element count: " + minCount + " to " + maxCount + ", value range: " + minValue + " to " +
                      maxValue + ", loops: " + loops);

            if (testCorrectness) Debug.Log("Testing Correctness.");
        }

        private void Update()
        {
            if (testCorrectness) TestCorrectness();
        }
        ...
        private void TestCorrectness()
        {
            ...
            var midSkip = Random.value < 0.5f ? testInt : 0;

            if (tuples1.Slinq().Aggregate(0, (acc, next) => acc + next.Item1) !=
                tuples1.Aggregate(0, (acc, next) => acc + next.Item1))
            {
                Debug.LogError("Aggregate failed.");
                testCorrectness = false;
            }

            if (tuples1.Slinq().Aggregate(0, (acc, next) => acc + next.Item1, acc => -acc) !=
                tuples1.Aggregate(0, (acc, next) => acc + next.Item1, acc => -acc))
            {
                Debug.LogError("Aggregate failed.");
                testCorrectness = false;
            }

            if (tuples1.Slinq().AggregateWhile(0,
                    (acc, next) => acc < testInt2 ? new Option<int>(acc + next.Item1) : new Option<int>()) != tuples1
                    .Slinq().AggregateRunning(0, (acc, next) => acc + next.Item1).Where(acc => acc >= testInt2)
                    .FirstOrDefault())
            {
            ...
            if (!tuples1.Slinq().Select(to_1).AverageOrNone()
                .Cata(avg => avg == tuples1.Select(to_1f).Average(), tuples1.Count == 0))
            {
            ...
            if (!new[] {tuples1, tuples2}.Slinq().Select(x => x.Slinq()).Flatten()
                .SequenceEqual(tuples1.Concat(tuples2).Slinq(), eq))
            {
            ...
            if (!tuples1.Slinq().Intersect(tuples2.Slinq(), eq_1)
                .SequenceEqual(tuples1.Intersect(tuples2, eq_1).Slinq(), eq))
            {
            ...
            if (!tuples1.Slinq().OrderByGroupDescending(to_1)
                .SequenceEqual(tuples1.OrderByDescending(to_1f).Slinq(), eq))
            {
...

            {
                ...
                for (var i = 0; i < testInt; ++i) slinq.Remove();
...

            {
                ...
                for (var i = 0; i < testInt; ++i) slinq.Remove();
                if (!list.SlinqDescending().Skip(midSkip)
                    .SequenceEqual(tuples1.SlinqDescending().Skip(midSkip).Skip(testInt), eq))
                {
...

            {
                ...
                if (!list.Slinq().Skip(midSkip)
                    .SequenceEqual(tuples1.Slinq().Skip(midSkip).SkipWhile(x => x.Item1 < testInt2), eq))
                {
...

            {
                ...
                var rAcc = list.Slinq().Skip(midSkip).RemoveWhile(0,
                    (acc, next) => acc < testInt2 ? new Option<int>(acc + next.Item1) : new Option<int>());
                var sAcc = sSlinq.SkipWhile(0,
                    (acc, next) => acc < testInt2 ? new Option<int>(acc + next.Item1) : new Option<int>());
            ...
            if (!new[] {tuples1, tuples2}.Slinq().SelectMany(x => x.Slinq())
                .SequenceEqual(tuples1.Concat(tuples2).Slinq(), eq))
            {
            ...
            if (!tuples1.Slinq().SelectMany(x => x.Item1 < testInt ? new Option<int>(x.Item1) : new Option<int>())
                .SequenceEqual(tuples1.Where(x => x.Item1 < testInt).Select(x => x.Item1).Slinq()))
            {
...

            {
                ...
                for (var i = 0; i < testInt; ++i) slinq.Skip();
            ...
            if (!tuples1.Slinq().SkipWhile(x => x.Item1 < testInt2)
                .SequenceEqual(tuples1.SkipWhile(x => x.Item1 < testInt2).Slinq(), eq))
            {
            ...
            if (!tuples1.SlinqDescending()
                .SequenceEqual(RemovableLinkedList().SlinqNodesDescending().Select(x => x.Value), eq))
            {
            ...
            if (!tuples1.Slinq().TakeWhile(x => x.Item1 < testInt2)
                .SequenceEqual(tuples1.TakeWhile(x => x.Item1 < testInt2).Slinq(), eq))
            {
                Debug.LogError("TakeWhile failed.");
                testCorrectness = false;
            }

            if (!tuples1.Slinq().TakeWhile((x, c) => x.Item1 < c, testInt2)
                .SequenceEqual(tuples1.TakeWhile(x => x.Item1 < testInt2).Slinq(), eq))
            {
                Debug.LogError("TakeWhile failed.");
                testCorrectness = false;
            ...
            if (!tuples1.Slinq().Where(x => x.Item1 < testInt)
                .SequenceEqual(tuples1.Where(x => x.Item1 < testInt).Slinq(), eq))
            {
                Debug.LogError("Where failed.");
                testCorrectness = false;
            }

            if (!tuples1.Slinq().Where((x, c) => x.Item1 < c, testInt)
                .SequenceEqual(tuples1.Where(x => x.Item1 < testInt).Slinq(), eq))
            {
                Debug.LogError("Where failed.");
                testCorrectness = false;
