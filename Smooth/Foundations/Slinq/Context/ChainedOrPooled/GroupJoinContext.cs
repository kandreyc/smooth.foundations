...
namespace Smooth.Slinq.Context
{
    ...
    public struct GroupJoinContext<U, K, T2, T, C>
    {
        ...
        public static Slinq<U, GroupJoinContext<U, K, T2, T, C>> GroupJoin(Lookup<K, T2> lookup, Slinq<T, C> outer,
            DelegateFunc<T, K> outerSelector, DelegateFunc<T, Slinq<T2, LinkedContext<T2>>, U> resultSelector,
            bool release)
        {
...
#pragma warning restore 0414
        ...
        private GroupJoinContext(Lookup<K, T2> lookup, Slinq<T, C> outer, DelegateFunc<T, K> outerSelector,
            DelegateFunc<T, Slinq<T2, LinkedContext<T2>>, U> resultSelector, bool release)
        {
            needsMove = false;
            this.lookup = lookup;
    ...
    public struct GroupJoinContext<U, K, T2, T, C, P>
    {
        ...
        public static Slinq<U, GroupJoinContext<U, K, T2, T, C, P>> GroupJoin(Lookup<K, T2> lookup, Slinq<T, C> outer,
            DelegateFunc<T, P, K> outerSelector, DelegateFunc<T, Slinq<T2, LinkedContext<T2>>, P, U> resultSelector,
            P parameter, bool release)
        {
            return new Slinq<U, GroupJoinContext<U, K, T2, T, C, P>>(
                skip,
                remove,
                dispose,
                new GroupJoinContext<U, K, T2, T, C, P>(lookup, outer, outerSelector, resultSelector, parameter,
                    release));
        }
...
#pragma warning restore 0414
        ...
        private GroupJoinContext(Lookup<K, T2> lookup, Slinq<T, C> outer, DelegateFunc<T, P, K> outerSelector,
            DelegateFunc<T, Slinq<T2, LinkedContext<T2>>, P, U> resultSelector, P parameter, bool release)
        {
            needsMove = false;
            this.lookup = lookup;
            this.outerSelector = outerSelector;
        ...
        private static void Skip(ref GroupJoinContext<U, K, T2, T, C, P> context, out Option<U> next)
        {
            ...
            if (context.chained.current.isSome)
            {
                ...
                next = new Option<U>(context.resultSelector(
                    context.chained.current.value,
                    context.lookup.GetValues(context.outerSelector(context.chained.current.value, context.parameter))
                        .SlinqAndKeep(bd),
                    context.parameter));
                bd.TryReleaseShared();
            }
