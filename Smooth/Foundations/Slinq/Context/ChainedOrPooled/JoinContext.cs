...
namespace Smooth.Slinq.Context
{
    ...
    public struct JoinContext<U, K, T2, T, C>
    {
        ...
        public static Slinq<U, JoinContext<U, K, T2, T, C>> Join(Lookup<K, T2> lookup, Slinq<T, C> outer,
            DelegateFunc<T, K> outerSelector, DelegateFunc<T, T2, U> resultSelector, bool release)
        {
...
#pragma warning restore 0414
        ...
        private JoinContext(Lookup<K, T2> lookup, Slinq<T, C> outer, DelegateFunc<T, K> outerSelector,
            DelegateFunc<T, T2, U> resultSelector, bool release)
        {
            needsMove = false;
            this.lookup = lookup;
            ...
            inner = chained.current.isSome &&
                    lookup.dictionary.TryGetValue(outerSelector(chained.current.value), out iht)
                ? iht.head
                : null;
        ...
        private static void Skip(ref JoinContext<U, K, T2, T, C> context, out Option<U> next)
        {
            ...
            else
            {
                next = new Option<U>();
                context.bd.Release();
                if (context.release) context.lookup.DisposeInBackground();
            }
        }
    ...
    public struct JoinContext<U, K, T2, T, C, P>
    {
        ...
        public static Slinq<U, JoinContext<U, K, T2, T, C, P>> Join(Lookup<K, T2> lookup, Slinq<T, C> outer,
            DelegateFunc<T, P, K> outerSelector, DelegateFunc<T, T2, P, U> resultSelector, P parameter, bool release)
        {
...
#pragma warning restore 0414
        ...
        private JoinContext(Lookup<K, T2> lookup, Slinq<T, C> outer, DelegateFunc<T, P, K> outerSelector,
            DelegateFunc<T, T2, P, U> resultSelector, P parameter, bool release)
        {
            needsMove = false;
            this.lookup = lookup;
            this.outerSelector = outerSelector;
            ...
            LinkedHeadTail<T2> iht;
            inner = chained.current.isSome &&
                    lookup.dictionary.TryGetValue(outerSelector(chained.current.value, parameter), out iht)
                ? iht.head
                : null;
        ...
        private static void Skip(ref JoinContext<U, K, T2, T, C, P> context, out Option<U> next)
        {
            ...
            if (context.inner == null && context.chained.current.isSome)
            {
                ...
                while (context.chained.current.isSome)
                {
                    context.inner = context.lookup
                        .GetValues(context.outerSelector(context.chained.current.value, context.parameter)).head;
                    if (context.inner == null)
                        context.chained.skip(ref context.chained.context, out context.chained.current);
                    else
                        break;
